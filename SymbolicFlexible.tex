...
\codeavailability{
    A Zenodo link will be created for the current version of:
    \url{https://github.com/ebranlard/yams}
...
$\m{K}_{0\omega,xy}^k$ is the geometrical stiffness matrix corresponding to a unit rotations in the $x$ and $y$ direction (assuming the coordinates of the rotational velocity are independent \comment{This is a bit odd, make sure I understand this}),
...
\weird{
See \autoref{sec:GeometricalStiffness}.
If a 1st order Taylor expansion is used (see Section XXX) for these different terms, then the unit geometrical stiffness matrix can directly be added to the first order terms since they are linear in the degrees of freedom $\v{q}_e$. For instance, let's assume that the mass matrix is given as a 1st order Taylor expansion: $\m{M}_{xe} = \m{M}_{xe}^0 + \m{M}_{xe}^1\v{q}_e$. Then the geometrical stiffening effect can directly be inserted into the first order term, such that $\m{M}_{xe}^1$ becomes $\m{M}_{xe}^1+\m{K}_{0t}$.
...
\todo{Write this section once we understand the partial load approach}
