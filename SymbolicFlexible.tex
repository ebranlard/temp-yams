...
The applications are multiple such as: time-domain simulation, stability analyses, frequency domain analyses, advanced controller design, state observers, digital twins, etc.
...
In practice, a zeroth or 1st order expansion is used.
The use of Taylor expansion is presented in \autoref{sec:TaylorExpansion}. 
...
\noindent\begin{figure}[!htb]\centering%
  \includegraphics[width=0.48\textwidth]{figs_svg/YAMSWorkflow.pdf}
...
The coefficients $k_\Omega$ of each degrees of freedom are obtained as: $\v{k}_\Omega =[1.7,\ 1.4,\ 5.5]$. We compare the frequencies obtained with the present method against OpenFAST linearization results in \autoref{fig:FrequencyCentrifugalStiffening}.
...
the tower is now represented by one shape function in both the fore-aft and side-side directions, $\v{\Phi}_1=\Phi_1 \v{\hat{x}}$ and $\v{\Phi}_2=\Phi_2\v{\hat{y}}$.
...
Sufficient level of accuracy was obtained using a 4th-order Runge-Kutta method which the default method.
Kane's method, which uses a minimal set of coordinates, tends to lead to stiff system, and it is possible that implicit integrator may be needed for other systems.
...
EB wrote a Python implementation and JG wrote a Maxima implementation.  EB wrote the main corpus of the paper, with feedback and contributions from JG.}
...
\codeavailability{
    ...
    \todoJens{A Zenodo link for CADyn (No rush on that, we can wait for the paper to be fully accepted..}
...
\end{align}
% \comment{should we mention the fundamental phenomenon, that axial stress in a beam causes it to become stiffer, like a string increases its pitch the tauter it is?}
% \comment{should we state the dependencies of $K_g(\dot(\v{\omega},\v{a},\v{\dot{\omega}},\v{p},\v{F}, \v{\tau} ) \ldots)$?}
...
% from the accelerations of the body, the external loads acting on the body and cross products of the rotational speed of the body (centrifugal and gyroscopic terms). 
...
% The accelerations are: and gravitational, $\v{g}$. \comment{and due to elastic mode accelerations?} 
    ...
    \sum\limits_{\alpha=1}^3 \left[ 
     ...
     + \sum\limits_{\alpha=1}^3\sum\limits_{\beta=1}^3  \omega_\alpha\omega_\beta \m{K}_{0\omega,\alpha\beta} 
      ...
      + \sum\limits_{k} 
       ...
       \label{eq:GeomStiff}
\end{align}
where the indices $\alpha$ and $\beta$ run on the $x, y, z$ coordinates. 
...
$\m{K}_{0\omega,xy}^k$ is the geometrical stiffness matrix corresponding to a unit rotations in the $x$ and $y$ direction (assuming the coordinates of the rotational velocity are independent \todoBoth{This is a bit odd, make sure I understand this}),
...
The different unit geometrical matrices introduced in \autoref{eq:StiffnessAxial} can be determined using a form \autoref{sec:GeometricalStiffness}, where the axial load $N$ is replaced by the unit inertial load external load. Since the beam is directed along the $z$ direction, we focus on the terms where the loads acts in the $z$ direction, all other terms being zero or negligible.
...
\todoEmmanuel{I'll rewrite this section based on our improved understanding of the Taylor expansion
Start by mentioning that the general integrals are valid when using integrals over the displaced body mass. Since we know the displacement field, we can factor the terms in front of the $q_e$.}
\todoEmmanuel{
The order of expansion of the displacement field leads to alternative formulations.
In \cite{shabana:book} and \cite{branlard:2019flex} a zeroth order expansion is used:
...
\todoEmmanuel{
The unit geometrical stiffness matrices (introduced in \autoref{sec:GeometricalStiffness}) can be directly integrated into the 1st order Taylor expansion terms..
se different terms, then the unit geometrical stiffness matrix can directly be added to the first order terms since they are linear in the degrees of freedom $\v{q}_e$. For instance, let's assume that the mass matrix is given as a 1st order Taylor expansion: $\m{M}_{xe} = \m{M}_{xe}^0 + \m{M}_{xe}^1\v{q}_e$. Then the geometrical stiffening effect can directly be inserted into the first order term, such that $\m{M}_{xe}^1$ becomes $\m{M}_{xe}^1+\m{K}_{0t}$.
Similarly, the term $\m{K}_{0r}$ can be inserted into $\m{M}_{\theta e}^1$, $\m{K}_{0\omega}$ into $\m{O}_e^1$, $\m{K}_{0F}$ into $\m{\Phi}^1$ and $\m{K}_{0\tau}$ into $\m{\Psi}^1$ in the calculation of the generalized forces. 
The different contributions are summarized in table 6.9 of the book of Schwertassek and Wallrapp \cite{Schwertassek:book}.
}
